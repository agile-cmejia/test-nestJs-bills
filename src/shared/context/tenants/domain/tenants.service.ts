...
export class TenantsService {
  ...
  async create(createTenantDto: CreateTenantDto) {
    this.logger.log('Creating new Tenants...');
  ...
  async findByMany(findTenantDto: FindTenantDto) {
    ...
    if (names && names.length > 0) {
      ...
      query = query.andWhere('Tenants.name IN (:...names) ', { names: names.map((name) => capitalizeSentence(name)) });
    }
    if (urls && urls.length > 0) {
      this.logger.log(`Finding by urls: ${urls}`);
      query = query.andWhere('Tenants.urls IN (:...urls) ', { urls: urls.map((url) => capitalizeSentence(url)) });
    }
    if (descriptions && descriptions.length > 0) {
      this.logger.log(`Finding by descriptions: ${descriptions}`);
      query = query.andWhere('Tenants.description IN (:...descriptions) ', { descriptions });
    }
    ...
    if (enabled != null) {
      ...
      query = query.andWhere('Tenants.enabled = :enabled ', { enabled });
    }

    this.tenantCollection = await query.getMany();
    this.logger.log(`Found ${this.tenantCollection.length} Tenants`);
    return this.tenantCollection;
  }
  ...
  async update(id: number, updateTenantDto: UpdateTenantDto) {
    ...
    this.tenantRepository.merge(tenantToUpdate, {
      ...updateTenantDto,
      name: capitalizeSentence(name).trim(),
      url: url.trim().toLowerCase(),
      description: description.trim(),
  ...
  async remove(id: number) {
    ...
    tenantToDisable.enabled = !tenantToDisable.enabled;
    this.logger.log(`Tenant with id: ${id} was ` + tenantToDisable.enabled ? 'Enabled!' : 'Disabled!');
